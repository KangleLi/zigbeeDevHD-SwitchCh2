  ...
  switch (zclCmd) {
    ...
    case ZCL_READ_ATTRIBUTES_COMMAND_ID:
    {
      ...
      while (msgIndex + 2 <= msgLen) {
        ...
        {
#include "_carelSDK/App/config.h"
#if TUYA_ENABLE
        if(clusterId == 0x0000){
        	if(attrId == 0x0001){
        		extern void _SEQ_Report_Add(uint8_t endpoint, uint16_t clusterId, uint16_t attributeId, bool ifMfg);
        		_SEQ_Report_Add(emberAfCurrentEndpoint(), clusterId, attrId, 0);
        	}
        }
#endif
        }
