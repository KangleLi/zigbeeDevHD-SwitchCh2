...
void emberAfOnOffClusterServerTickCallback(uint8_t endpoint)
{
	...
	extern void simonExtendCmd_Timer_EF(void);
...
{
	// carel C (自定义定时处理)
	extern bool _carel_emberAfOnOffClusterOnWithTimedOffCallback(uint8_t onOffControl,
            uint16_t onTime,
            uint16_t offWaitTime);
	return _carel_emberAfOnOffClusterOnWithTimedOffCallback(onOffControl,
			   onTime,
			   offWaitTime);
  ...
  if (0x0000 < offWaitTimeAttribute && !onOffAttribute) {
    ...
    // carel + (确保ontime上报)
    if((onTimeAttribute % 60) != 0){
    	_reportStdAttribute(0, endpoint, 1, 0x0006, 0x4001, 1);
    }
    // 20190911
    if (status != EMBER_ZCL_STATUS_SUCCESS) {
      goto kickout;
